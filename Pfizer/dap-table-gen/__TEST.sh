...
[[ -e "DAP.TBL.QRY.06_out.properties" ]] && rm DAP.TBL.QRY.06_out.properties
...
echo
echo "------------------------------"
boomi -d "$dir_table".dat -p DAP.TBL.STR.01_out.properties DAP.TBL.QRY.06__PREPARE_DbSelect.b.groovy
# grep -v '.*Config.*' DAP.TBL.QRY.06_out.properties | sed 's/document\.dynamic\.userdefined\.//'
...
# boomi -d ./DAP.TBL.STR.03_out.dat -p DAP.TBL.STR.03_out.properties DAP.TBL.FMT.05__APPLY_ColumnWidths.b.groovy
# boomi -d ./DAP.TBL.STR.04_out.dat -p DAP.TBL.STR.04_out.properties DAP.TBL.FMT.05__APPLY_ColumnWidths.b.groovy
# grep -v '.*Config.*' DAP.TBL.STR.04_out.properties | sed 's/document\.dynamic\.userdefined\.//'

# [[ -e "DAP.TBL.STR.02_out.properties" ]] && \
# cat DAP.TBL.STR.02_out.properties | sed 's/document\.dynamic\.userdefined\.//'

[[ -e "DAP.TBL.STR.03_out.dat" ]] && mv DAP.TBL.STR.03_out.dat DAP.TBL.STR.03_out.html
[[ -e "DAP.TBL.STR.04_out.dat" ]] && mv DAP.TBL.STR.04_out.dat DAP.TBL.STR.04_out.html
[[ -e "DAP.TBL.STR.05_out.dat" ]] && mv DAP.TBL.STR.05_out.dat DAP.TBL.STR.05_out.html
